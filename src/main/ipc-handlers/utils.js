    ...
    return new Promise((resolve) => {
      try {
        ...
        const options = {
          ...
          path: "/repos/Kopw/Lvory/releases",
          method: "GET",
          headers: {
          ...
          res.on("end", () => {
            try {
              ...
              if (stableReleases.length > 0) {
                ...
                const latestStableVersion = latestStable.tag_name.replace(
                  /^v/,
                  "",
                );
              ...
              if (isDevelopmentBuild && nightlyReleases.length > 0) {
                ...
                const latestNightlyDate = latestNightly.tag_name.replace(
                  /^nightly-/,
                  "",
                );
              ...
              logger.info(
                `检查更新: 当前版本=${currentVersion}, 最新版本=${latestVersion}, 有更新=${hasUpdate}, 类型=${updateType}`,
              );
  ...
  ipcMain.handle("get-network-interfaces", async () => {
    try {
      ...
      Object.keys(networkInterfaces).forEach((interfaceName) => {
        // 过滤IPv4地址并排除本地回环
        const interfaces = networkInterfaces[interfaceName].filter(
  ...
  ipcMain.handle("get-all-versions", async () => {
    return new Promise((resolve) => {
      try {
        const options = {
          hostname: "api.github.com",
          path: "/repos/Kopw/Lvory/releases?per_page=50",
          method: "GET",
          headers: {
            "User-Agent": "lvory-updater",
            Accept: "application/vnd.github.v3+json",
